...
const formatPrice = (price: any, type: ListingType, priceHidden?: boolean): string | string[] => {
    ...
    if (type === 'rental') {
        const rates = [];
        if (pricingData.hourlyRate) rates.push(`${formatter.format(pricingData.hourlyRate)} / saat`);
    ...
    const handleTouchEnd = (e: React.TouchEvent) => {
        ...
        if (Math.abs(diff) > 40) {
            if (diff > 0) {
                ...
                onIndexChange(activeIndex - 1 >= 0 ? activeIndex - 1 : images.length - 1);
            }
        }
        setTouchStartX(null);
    };
    ...
    return (
        ...
        >
            ...
            <button
                ...
                className="absolute left-2 sm:left-4 z-10 text-white hover:text-primary p-2 bg-black/40 rounded-full backdrop-blur-sm"
            >
                <ChevronLeft className="h-7 w-7 sm:h-10 sm:w-10" />
            </button>

            <div className="relative w-full h-full max-w-7xl max-h-[85vh] flex items-center justify-center px-12 sm:px-16">
                <Image
            ...
            <button
                ...
                className="absolute right-2 sm:right-4 z-10 text-white hover:text-primary p-2 bg-black/40 rounded-full backdrop-blur-sm"
            >
                <ChevronRight className="h-7 w-7 sm:h-10 sm:w-10" />
            </button>

            <div className="absolute bottom-6 left-0 right-0 flex flex-col items-center gap-2">
                <span className="text-white/70 text-xs font-medium">{activeIndex + 1} / {images.length}</span>
                <div className="flex justify-center gap-1.5">
                    {images.map((_, idx) => (
                        <button
                            key={idx}
                            onClick={(e) => { e.stopPropagation(); onIndexChange(idx); }}
                            className={cn("h-1.5 rounded-full transition-all", activeIndex === idx ? "bg-primary w-5" : "bg-white/40 w-1.5 hover:bg-white/70")}
                        />
                    ))}
                </div>
            </div>
...
export default function ListingDetailClient({ listing, type }: ListingDetailProps) {
    ...
    return (
        <>
            ...
            <div className="fixed bottom-0 left-0 w-full z-50 lg:hidden" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
                <div className="bg-[#0A0A0A] border-t border-white/10 px-4 pt-3 pb-4">
                    {/* Price row */}
                    <div className="flex items-baseline gap-2 mb-3">
                        <span className="text-[10px] font-semibold text-neutral-500 uppercase tracking-wider">Fiyat</span>
                        <span className="text-xl font-black font-oswald text-primary leading-none">
                            {(() => {
                        ...
                        {(() => {
                            ...
                            if (arr.length > 1) {
                                return <span className="text-xs text-neutral-500">{arr[1]}</span>;
                            }
                            return null;
                        })()}
                    </div>
                    ...
                    <div className="flex gap-2">
                        <AppGuard trigger="message" productImage={images[0] ? getImageUrl(images[0]) : undefined} className="flex-1">
                            <button className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-white/20 bg-white/5 py-3 text-white font-bold text-sm active:scale-95 transition-transform">
                        ...
                        {phoneVisible && publicPhone ? (
                            <>
                                <AppGuard trigger="message" productImage={images[0] ? getImageUrl(images[0]) : undefined} className="flex-1">
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-green-500/40 bg-green-500/10 py-3 text-green-400 font-bold text-sm active:scale-95 transition-transform"
                                    >
                                        <WhatsAppIcon className="h-4 w-4 shrink-0" />
                                        <span>WhatsApp</span>
                                    </a>
                                </AppGuard>
                                <AppGuard trigger="call" productImage={images[0] ? getImageUrl(images[0]) : undefined} className="flex-1">
                                    <a
                                        href={`tel:${publicPhone.replace(/\s/g, '')}`}
                                        className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-primary py-3 text-white font-bold text-sm shadow-lg shadow-primary/20 active:scale-95 transition-transform"
                                    >
                            ...
                            <AppGuard trigger="call" productImage={images[0] ? getImageUrl(images[0]) : undefined} className="flex-[2]">
                                <button className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-primary py-3 text-white font-bold text-sm shadow-lg shadow-primary/20 active:scale-95 transition-transform">
                                    <Phone className="h-4 w-4 shrink-0" />
